...
export const deleteOrderItem = async (order_id: string, orderItem_id: string) => {
  const access_token = localStorage.getItem("access_token");
  if (access_token)
    try {
      const response = await api.delete(`/clients/orders/${order_id}/order_items/${orderItem_id}`,{
        headers: {
          Authorization: `Bearer ${access_token}`
        }
      });
...
export const deleteChefTelephone = async (chef_id: string, telephone_id: string) => {
  const access_token = localStorage.getItem("access_token");
  if (access_token)
    try {
      const response = await api.delete(`/chefs/${chef_id}/telephones/${telephone_id}`,{
        headers: {
          Authorization: `Bearer ${access_token}`
        }
      });
