...
export const getCities = async (state_id: string) => {
    ...
    if (access_token)
      try {
        const response = await api.get<CitiesResponseType>(`/states/${state_id}/cities`,{
          headers: {
            Authorization: `Bearer ${access_token}`
          }
        });
...
export const getCity = async (state_id: string, city_id: string) => {
    ...
    if (access_token)
      try {
        const response = await api.get<CityType>(`/states/${state_id}/cities/${city_id}`,{
          headers: {
            Authorization: `Bearer ${access_token}`
          }
        });
...
export const getAddressByZipCode = async (zip_code: string) => {
    ...
    if (access_token)
      try {
        const response = await api.get<AddressByZipCodeType>(`/addresses/search_zip_code/${zip_code}`,{
          headers: {
            Authorization: `Bearer ${access_token}`
          }
        });
...
export const getClientsAddress = async () => {
    ...
    if (access_token)
      try {
        const response = await api.get<ClientsAddressResponseType>("/clients/addresses",{
          headers: {
            Authorization: `Bearer ${access_token}`
          }
        });
...
export const getChef = async (chef_id: string) => {
    ...
    if (access_token)
      try {
        const response = await api.get<ChefType>(`/chefs/${chef_id}`,{
          headers: {
            Authorization: `Bearer ${access_token}`
          }
        });
...
export const getChefAddress = async (chef_id: string, address_id: string) => {
    ...
    if (access_token)
      try {
        const response = await api.get<AddressResponseType>(`/chefs/${chef_id}/addresses/${address_id}`,{
          headers: {
            Authorization: `Bearer ${access_token}`
          }
        });
...
export const getOrder = async (order_id: string) => {
    ...
    if (access_token)
      try {
        const response = await api.get<OrderResponseType>(`/clients/orders/${order_id}`,{
          headers: {
            Authorization: `Bearer ${access_token}`
          }
        });
...
export const getChefOrders = async (chef_id: string) => {
    ...
    if (access_token)
      try {
        const response = await api.get<OrdersResponseType>(`/chefs/${chef_id}/orders`,{
          headers: {
            Authorization: `Bearer ${access_token}`
          }
        });
...
export const getChefOrder = async (chef_id: string, order_id: string) => {
    ...
    if (access_token)
      try {
        const response = await api.get<OrdersResponseType>(`/chefs/${chef_id}/orders/${order_id}`,{
          headers: {
            Authorization: `Bearer ${access_token}`
          }
        });
...
export const getOrderItems = async (order_id: string) => {
  ...
  if (access_token)
    try {
      const response = await api.get<OrderItemResponseType[]>(`/clients/orders/${order_id}/order_items`,{
        headers: {
          Authorization: `Bearer ${access_token}`
        }
      });
...
export const getOrderItem = async (order_id: string, orderItem_id: string) => {
  ...
  if (access_token)
    try {
      const response = await api.get<OrderItemResponseType>(`/clients/orders/${order_id}/order_items/${orderItem_id}`,{
        headers: {
          Authorization: `Bearer ${access_token}`
        }
      });
...
export const getClientTelephones = async () => {
  ...
  if (access_token)
    try {
      const response = await api.get<TelephonesResponseType>("/clients/telephones",{
        headers: {
          Authorization: `Bearer ${access_token}`
        }
      });
...
export const getClientTelephone = async (telephone_id: string) => {
  ...
  if (access_token)
    try {
      const response = await api.get<TelephoneType>(`/clients/telephones/${telephone_id}`,{
        headers: {
          Authorization: `Bearer ${access_token}`
        }
      });
...
export const getChefTelephones = async (chef_id: string) => {
  ...
  if (access_token)
    try {
      const response = await api.get<TelephonesResponseType>(`/chefs/${chef_id}/telephones`,{
        headers: {
          Authorization: `Bearer ${access_token}`
        }
      });
...
export const getChefTelephone = async (chef_id: string, telephone_id: string) => {
  const access_token = localStorage.getItem("access_token");
  if (access_token)
    try {
      const response = await api.get<TelephoneType>(`/chefs/${chef_id}/telephones/${telephone_id}`,{
        headers: {
          Authorization: `Bearer ${access_token}`
        }
      });
...
export const getChefDishes = async (chef_id: string) => {
  ...
  if (access_token)
    try {
      const response = await api.get<DishesResponseType>(`/chefs/${chef_id}/dishes`,{
        headers: {
          Authorization: `Bearer ${access_token}`
        }
      });
...
export const getChefDish = async (chef_id: string, dish_id: string) => {
  ...
  if (access_token)
    try {
      const response = await api.get<DishType>(`/chefs/${chef_id}/dishes/${dish_id}`,{
        headers: {
          Authorization: `Bearer ${access_token}`
        }
      });
