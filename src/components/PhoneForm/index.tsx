...
export const PhoneForm = ({id, number}: PhoneFormProps) => {
    ...
    if(number && id){
        ...
        return(
            ...
            {({ errors, touched }) => (
                <Form className={styles.formContent}>
                  <Input name="number" id="number" type="text" errors={touched.number && errors.number}/>
                  <section className={styles.ButtonAndMessage}>
                  <ButtonForm text="Enviar" type="submit"/>
                  {showSuccessMessage && <p className={styles.successMessage}>{successMessages.phone.update}</p>}
        ...
        const onSubmitAdd = async(values: TelephoneUpdateType, helper: FormikHelpers<TelephoneUpdateType>)=>{
        ...
        {({ errors, touched }) => (
            <Form className={styles.formContent}>
              <Input name="number" id="number" type="text" errors={touched.number && errors.number} placeholder="Digite um telefone"/>
              <section className={styles.ButtonAndMessage}>
              <ButtonForm text="Enviar" type="submit"/>
              {showSuccessMessage && <p className={styles.successMessage}>{successMessages.phone.add}</p>}
