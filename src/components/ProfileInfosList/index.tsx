    ...
    if (user)
        return (
            <div className={styles.infosContent}>
                ...
                {isOpenPhone && <Modal isOpen={isOpenPhone} setIsOpen={setIsOpenPhone} title="Adicionar telefone">
                <PhoneForm/>
                </Modal>}
                {isOpenAddress && <Modal isOpen={isOpenAddress} setIsOpen={setIsOpenAddress} title="Adicionar endereço">
                <AddressForm/>
                </Modal>}
                ...
                {title === "Telefones" ? (
                    <div className={styles.listAndAddButton}>
                        <div className={styles.infosList}>
                            {data.map((value) => (
                                <TelephoneListLine phone={value} />
                            ))}
                        </div>
                        <button className={styles.addButton}  onClick={onClickModalPhone}>Adicionar</button>
                    </div>
                ) : (
                    <div className={styles.listAndAddButton}>
                        <div className={styles.infosList}>
                            {data.map((value) => (
                                <AddressListLine address={value as AddressResponseType} />
                            ))}
                        </div>
                        <button className={styles.addButton} onClick={onClickModalAddress}>Adicionar</button>
                    </div>
                )}
